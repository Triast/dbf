# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.2.1] - 2026-05-06

### Fixed

- Discarding database backlink in `Table.init`.
- Various compilation errors.

## [0.2.0] - 2026-05-05

### Added

- Options for `Table` to ignore deleted record and trim `[]const u8`.
- `structIter(gpa: std.mem.Allocator, T: type)` method to `Table`, which makes possible to iterate over a DBF table with each row as an instance of type `T`.

### Updated

- Updated `minimum_zig_version` version to `0.16.0`.

## [0.1.0] - 2026-02-24

### Added

- Iteration over opened DBF table.
- Reading fields as zig types:
  - `[]const u8`,
  - `u8`,
  - `u16`,
  - `u32`,
  - `u64`,
  - `f16`,
  - `f32`,
  - `f64`,
  - `f80`,
  - `f128`,
  - `dbf.Date`,
  - `bool`.

[0.2.1]: https://codeberg.org/Triast/dbf/compare/v0.2.0...v0.2.1
[0.2.0]: https://codeberg.org/Triast/dbf/compare/v0.1.0...v0.2.0
[0.1.0]: https://codeberg.org/Triast/dbf/src/tag/v0.1.0
